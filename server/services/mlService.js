...
const getMLPrediction = async (transaction) => {
  try {
    ...
    // Since our transactions don't have V1-V28 features,
    // we'll derive proxy features from what we have
    ...
    // We use amount and derive V features from transaction properties
    const payload = {
      amount: amount,
      time: new Date(transaction.timestamp).getHours() * 3600,
      V1: isAbroad * -3.5,           // foreign location signal
      V2: isNightTime * -2.8,        // night time signal
      V3: isHighRiskCategory * -3.2, // high risk category signal
      V4: isRoundAmount * -2.1,      // round amount signal
      V5: amount > 100000 ? -3.8 : 0.5, // large amount signal
      V6: amount > 50000 ? -2.5 : 0.3,  // medium-large amount signal
      V7: 0.2,
      V8: -0.1,
      V9: 0.3,
      V10: -0.2,
      ...
      V12: -0.3,
      V13: 0.2,
      V14: isAbroad * -4.1,          // strongest fraud signal
      V15: 0.1,
    ...
    // If ML API is down, fall back to rule engine only
